...
# DB생성 & Autocommit
# 본인 DB 파일 경로
conn = sqlite3.connect('C:/python_basic/resources/records.db', isolation_level=None)

# Cursor연결
cursor = conn.cursor()

# 테이블 생성(Datatype : TEXT NUMERIC INTEGER REAL BLOBS)
cursor.execute(
...
with open('./resources/word.txt', 'r') as f:  # 문제 txt 파일 로드
    for c in f:
